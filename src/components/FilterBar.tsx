...
const sortOptions = [
  { value: "nearest", label: "Gần nhất", icon: MapPin },
  { value: "rating", label: "Đánh giá cao", icon: Star },
  { value: "hottest", label: "Xu hướng", icon: TrendingUp },
  { value: "shortest_wait", label: "Thời gian chờ ngắn", icon: Clock },
  { value: "pre_order", label: "Có thể đặt trước", icon: CalendarClock },
] as const;

const cuisineTypes = ["Ý", "Trung Quốc", "Ấn Độ", "Mexico", "Nhật Bản", "Thái Lan", "Mỹ", "Địa Trung Hải", "Việt Nam", "Hàn Quốc"];

const priceRanges = [
  { value: "€", label: "Tiết kiệm (€)" },
...
export function FilterBar({ onFilterChange }: FilterBarProps) {
  ...
  return (
    <div className="bg-card border-b sticky top-16 z-40 py-3">
              ...
              <DropdownMenuContent className="w-48 max-h-64 overflow-y-auto">
                {cuisineTypes.map(cuisine => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      "cursor-pointer",
                      selectedCuisines.includes(cuisine) && "bg-accent text-accent-foreground"
                    )}
                  >
                    {cuisine}
              ...
              <DropdownMenuContent>
                {priceRanges.map(({ value, label }) => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      "cursor-pointer",
                      selectedPrices.includes(value) && "bg-accent text-accent-foreground"
                    )}
                  >
                    {label}
