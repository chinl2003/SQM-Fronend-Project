...
type OrderWithDetailsDto = {
  ...
  status: OrderStatusNumber | string | null;
  createdAt?: string;
...
function unwrapOrders<T = OrderWithDetailsDto>(raw): T[] {
...
function statusToLabel(s?: number | string) {
  ...
  switch (n) {
    ...
    case "indebt":
      return {
        text: "Trạng thái: Quá hạn thanh toán công nợ",
        dot: "bg-orange-500",
      };
    case "suspended":
...
async function updateOrderStatusOnServer(
  ...
  const payload = {
    id: String(orderId),
    status: apiStatusFromUi(nextUiStatus),
  };
...
type QueueUiItem = {
  ...
  status:
    ...
    | "cancelled";
  time: string;
  type: "walk-in" | "pre-order";
  paymentStatus: "paid" | "unpaid";
  paymentMethod: "cash" | "vnpay";
};

function uiStatusFromApi(
  ...
  if (t === "processing" || t === "prepareing" || t === "preparing")
    return "preparing";
...
function mapOrderToQueueUi(o: OrderWithDetailsDto): QueueUiItem {
  const ui = uiStatusFromApi(o.status);
  ...
  return {
    ...
    status: uiForList as QueueUiItem["status"],
    time: o.createdAt
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled)
          setBanksError(
            "Không tải được danh sách ngân hàng. Vui lòng thử lại."
          );
      } finally {
...
const VendorDashboard = () => {
  ...
  const [loadQueueError, setLoadQueueError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function fetchVendorByOwner() {
      try {
        ...
        if (!userId) {
          throw new Error(
            "userId is required but was not found (route or localStorage)"
          );
        }
        const token = localStorage.getItem("accessToken") || "";
  ...
  async function fetchQueueForVendor(vendorId: string) {
    try {
      setLoadingQueue(true);
      setLoadQueueError(null);
      const token = localStorage.getItem("accessToken") || "";
      const headers = token ? { Authorization: `Bearer ${token}` } : undefined;

      const res = await api.get(
        `/api/order/by-customer?vendorId=${encodeURIComponent(vendorId)}`,
        headers
      );
      ...
      console.error(e);
      setLoadQueueError(e?.message || "Không thể tải hàng đợi.");
  ...
  useEffect(() => {
    if (vendor?.id && activeTab === "queue") {
      fetchQueueForVendor(vendor.id);
    }
  }, [vendor?.id, activeTab]);
  ...
  const handleUpdateOrder = (orderId: number | string, newStatus: string) => {
    setQueueItems((prev) =>
      prev.map((item) =>
        item.id === orderId ? { ...item, status: newStatus as any } : item
      )
    );
  };
  ...
  const getPaymentStatusColor = (status: string) => {
    return status === "paid"
      ? "bg-chart-secondary text-chart-secondary-foreground"
      : "bg-queue-cancelled text-queue-cancelled-foreground";
  };
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="text-muted-foreground flex items-center gap-2">
              ...
              {loadError && (
                <span className="text-destructive ml-2">{loadError}</span>
              )}
            </p>
          </div>
          ...
          <TabsContent value="queue" className="space-y-4">
            <Card className="shadow-md">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      size="icon"
                      onClick={() =>
                        vendor?.id && fetchQueueForVendor(vendor.id)
                      }
              ...
              <CardContent>
                ...
                <div className="space-y-3">
                  {sortedQueueItems.length === 0 && !loadingQueue ? (
                    <div className="text-sm text-muted-foreground">
                      Chưa có đơn nào trong hàng đợi.
                    </div>
                  ) : (
                    sortedQueueItems.map((item) => (
                      <div
                        key={item.id}
                        className="border border-border rounded-lg p-4 cursor-pointer hover:bg-muted/30 transition-colors relative overflow-hidden"
                        onClick={() => setSelectedOrder(item)}
                      >
                        <div
                          className={`${getPaymentStatusColor(
                            item.paymentStatus
                          )} absolute top-0 right-0 px-2 py-1 text-[10px] font-medium rounded-br-none rounded-tl-none rounded-tr-lg rounded-bl-lg`}
                        >
                                    ...
                                    <Button
                                      ...
                                      onClick={async (e) => {
                                        ...
                                        try {
                                          ...
                                          handleUpdateOrder(item.id, nextUi);
                                          toast.success(
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  ...
                                  <DropdownMenuItem
                                    ...
                                    onClick={() =>
                                      handleUpdateOrder(item.id, "cancelled")
                                    }
                                  >
                                    Hủy đơn hàng
                                  </DropdownMenuItem>
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mb-2">
                            "Món ăn ngon, phục vụ nhanh. Rất hài lòng với dịch
                            vụ."
                          </p>
                          <p className="text-xs text-muted-foreground">
                            2 giờ trước
              ...
              <div className="flex gap-3 justify-end">
                ...
                <Button
                  onClick={() => {
                    ...
                    alert(
                      "Đã cập nhật thời gian trễ. Khách hàng sẽ được thông báo."
                    );
                  }}
                >
                  Xác nhận
