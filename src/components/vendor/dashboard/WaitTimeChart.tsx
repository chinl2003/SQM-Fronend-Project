    ...
    return (
      <div className="bg-card border border-border rounded-lg p-3 shadow-lg">
        ...
        <p className="text-sm text-primary">
          Thời gian chờ: <span className="font-medium">{payload[0].value} phút</span>
        </p>
...
export function WaitTimeChart() {
   const mostDelayedDish = "Phở bò tái";
  return (
    <div className="chart-container animate-fade-in" style={{ animationDelay: "0.1s" }}>
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Phân tích thời gian chờ trung bình trong ngày
          </p>
        </div>
          ...
          <TooltipProvider delayDuration={150}>
            <UiTooltip>
              <TooltipTrigger asChild>
                <div className="flex items-center gap-1 px-3 py-1 rounded-full border border-[#FCA5A5] bg-[#FFF5F5]">
                  <AlertTriangle className="w-3.5 h-3.5 text-[#EF4444]" />
                  <span className="text-[11px] sm:text-xs font-semibold text-[#7F1D1D]">
                    {mostDelayedDish}
                  </span>
                </div>
              </TooltipTrigger>
              ...
              <TooltipContent
                side="bottom"
                className="bg-[#1F130A] text-white text-xs py-1 px-2 rounded-md shadow-md"
              >
